...
def not_converged(wn, tau, g_0_wn_up, g_0_wn_dn, g_0_tau_up, g_0_tau_dn,
                 ...
                 g_wn_up, g_wn_dn, loop, U, mu):
    # Print g_0_wn
    generic(wn, g_0_wn_up, g_0_wn_dn, 
            r'$\omega_n$', r'$G_0(\omega_n)$', 
            "./figures/not_converged/g_0_wn_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+"_loop="+str(loop)+".pdf")  
    
    # Write g_0_wn
    file = open("./data/not_converged/g_0_wn_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+"_loop="+str(loop)+".txt", "w") 
    ...
    generic(tau, g_0_tau_up, g_0_tau_dn, 
            r'$\tau$', r'$G_0(\tau)$', 
            "./figures/not_converged/g_0_tau_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+"_loop="+str(loop)+".pdf") 
    
    # Write g_0_tau     
    file = open("./data/not_converged/g_0_tau_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+"_loop="+str(loop)+".txt", "w") 
    ...
    generic(wn, sigma_wn_up, sigma_wn_dn, 
            ...
            "./figures/not_converged/sig_wn_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+"_loop="+str(loop)+".pdf")
            
    # Write sigma_wn
    file = open("./data/not_converged/sig_wn_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+"_loop="+str(loop)+".txt", "w") 
    ...
    generic(tau, sigma_tau_up, sigma_tau_dn, 
            r'$\tau$', r'$\Sigma(\tau)$', 
            "./figures/not_converged/sig_tau_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+"_loop="+str(loop)+".pdf")
            
    # Write sigma_tau
    file = open("./data/not_converged/sig_tau_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+"_loop="+str(loop)+".txt", "w") 
    ...
    generic(wn, g_wn_up, g_wn_dn, 
            ...
            "./figures/not_converged/g_wn_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+"_loop="+str(loop)+".pdf")
            
    # Write g_wn
    file = open("./data/not_converged/g_wn_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+"_loop="+str(loop)+".txt", "w") 
...
def dos(beta_print, w, dos_U, U_print, mu_list, y_labels, hyst):
    for i in range(len(beta_print)):
        ...
        for j in range(plots):
            ...
            for k in range(len(y_labels)):
                ...
                ax.set(ylabel=r'$\rho(\omega)$')
                ax.plot(w, dos[k][j], label=y_labels[k])
            plt.xlim(-4, 4) #plt.xlim(-4, 1.5) 
            plt.ylim(0, 3)      
            plt.legend()

            plt.savefig("./figures/dos/dos_beta="+f'{beta:.3}'+
                        "_U="+f'{U_print[j]:.3}'+"_mu="+f'{mu_list[j]:.3}'+".pdf")
            plt.close()

# Print the Green functions
def green_func(beta_print, tau_U, g_wn_U, g_tau_U, 
    ...
    for i in range(len(beta_print)):
        ...
        for j in range(len(U_print)):
            U = U_print[j]
            mu = mu_list[j] # Assuming there are as many mu values as U
            ...
            plt.scatter(wn, g_wn_0[j].imag, s=1,  label=g_labels[0] + ' Im')
            plt.scatter(wn, g_wn_0[j].real, s=1,  label=g_labels[0] + ' Re')
            plt.scatter(wn, g_wn_1[j].imag, s=1,  label=g_labels[1] + ' Im')
            plt.scatter(wn, g_wn_1[j].real, s=1,  label=g_labels[1] + ' Re')
            plt.legend()
            plt.title(r"$\beta$ = "+f'{beta:.3}'+" U = "+f'{U:.3}'+' mu = '+f'{mu:.3}'+branch)
            plt.savefig("./figures/g_wn/g_wn_beta="+f'{beta:.3}'+"_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+branch+".pdf")
            plt.close()
            
            file = open("./data/g_wn_beta="+f'{beta:.3}'+"_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+".txt", "w") 
            file.write("wn\tg_wn_up\tg_wn_dn\n")
            for w, g_0, g_1 in zip(wn, g_wn_0[j], g_wn_1[j]):
                file.write(str(w) + "\t" + str(g_0) + "\t" + str(g_1) + "\n")
            file.close()
            ...
            plt.xlabel(r'$\tau$')
            plt.ylabel(r'$g(\tau)$')
            plt.scatter(tau, g_tau_0[j].imag, s=1, label=g_labels[0] + ' Im')
            plt.scatter(tau, g_tau_0[j].real, s=1, label=g_labels[0] + ' Re')
            plt.scatter(tau, g_tau_1[j].imag, s=1, label=g_labels[1] + ' Im')
            plt.scatter(tau, g_tau_1[j].real, s=1, label=g_labels[1] + ' Re')
            plt.legend()
            plt.title(r"$\beta$ = "+f'{beta:.3}'+" U = "+f'{U:.3}'+' mu = '+f'{mu:.3}'+branch)
            plt.savefig("./figures/g_tau/g_tau_beta="+f'{beta:.3}'+"_U="+f'{U:.3}'+'_mu='+f'{mu:.3}'+branch+".pdf")
            plt.close()
